return function(settings)
    -- SETTINGS (only two for simplicity)
    local cooldown     = settings.cooldown or 3         -- seconds before next flip
    local markerUpdate = settings.markerUpdate or 0.1   -- how often landing marker updates
    ...
    -- 🔄 Update marker unless locked
    task.spawn(function()
        while task.wait(markerUpdate) do
    ...
    -- ✅ Keybinds
    function zeezyFrontflip(act, inp)
    ...
    function zeezyBackflip(act, inp)
        if inp == Enum.UserInputState.Begin and canFlip then
            performFlip(-1)
            startCooldown()
        end
